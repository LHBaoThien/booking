@if (isset($sessionCart))
    @if (Session::has($sessionCart) && isset($datepicker1) && isset($datepicker2))
        @foreach (Session::get($sessionCart)->product as $item)
            <ul class="popular-hotels order-detail">
                <li>
                    ...
                    <button class="btn_del_cart"
                        onclick="DelItem({{ $item['productInfo']->id }} , {{ $item['productInfo']->homestay->id }}, '{{ $datepicker1 }}', '{{ $datepicker2 }}' )"
                        title="Xóa">X</button>
                </li>
            </ul>
        @endforeach
        <ul class="popular-hotels order-sum">
            <li>
                <h3>Tổng tiền: <span
                        class="payment">{{ number_format(Session::get($sessionCart)->totalPrice, 0, ',', '.') }}đ</span>
                </h3>
...
@else
    @if (Session::has('Cart-homestay-' . $homestayVal->id) && isset($datepicker1) && isset($datepicker2))
        ...
        <ul class="popular-hotels order-sum">
            <li>
                <h3>Tổng tiền: <span
                        class="payment">{{ number_format(Session::get('Cart-homestay-' . $homestayVal->id)->totalPrice, 0, ',', '.') }}đ</span>
                </h3>
