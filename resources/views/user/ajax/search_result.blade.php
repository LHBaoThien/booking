@if (count($product))
    @foreach ($product as $productVal)
        ...
        @endif
        <figure><a href="{{ route('userRoomDetail') . '?id=' . $productVal->homestay->id . $url }}" title=""><img
                    src="{{ $productVal->homestay->avatar }}" alt="" width="270" height="152" /></a></figure>
        <div class="details">
            ...
            <span class="address"><a
                    href="{{ route('userSearch') . '?address=' . $productVal->homestay->province->name . $url }}">{{ $productVal->homestay->province->name }}</a></span>
            <span class="address"><a
                    href="{{ route('userSearch') . '?address=' . $productVal->homestay->district->name . ' - ' . $productVal->homestay->province->name . $url }}">{{ $productVal->homestay->district->name }}</a></span>
            ...
            <a href="{{ route('userRoomDetail') . '?id=' . $productVal->homestay->id . $url }}" title="Book now"
                class="gradient-button">Chọn phòng</a>
        </div>
        </article>
        <!--//deal-->
    @endforeach
