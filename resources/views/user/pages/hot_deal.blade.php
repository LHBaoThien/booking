            ...
            <div class="content clearfix">
                ...
                <section class="full">
                    @foreach ($matp as $matpVal)
                        <h1 class="address-hot-deal"><a
                                href="{{ route('userSearch') . '?address=' . $matpVal->province->name . $urlSearch }}">{{ $matpVal->province->name }}</a>
                        </h1>
                                ...
                                <article class="one-fourth promo">
                                    ...
                                    <div class="details">
                                        ...
                                        <span class="address"><a
                                                href="{{ route('userSearch') . '?address=' . $homestayVal->province->name . $urlSearch }}">{{ $homestayVal->province->name }}</a></span>
                                        <span class="address"><a
                                                href="{{ route('userSearch') . '?address=' . $homestayVal->district->name . ' - ' . $homestayVal->province->name . $urlSearch }}">{{ $homestayVal->district->name }}</a></span>
                                        ...
                                        <a href="{{ route('userRoomDetail') . '?id=' . $homestayVal->id . $urlSearch }}"
                                            title="Book now" class="gradient-button">Chọn phòng</a>
                                    </div>
                                </article>
                                <!--//deal-->
                            @endforeach
