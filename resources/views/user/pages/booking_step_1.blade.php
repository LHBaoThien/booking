        ...
        <div class="wrap clearfix">
            ...
            <div class="content clearfix">
                ...
                <section class="three-fourth">
                            ...
                            <fieldset>
                                ...
                                <div class="row">
                                    <div class="f-item checkbox" onclick="checked()">
                                        ...
                                        <label>Có, tôi đã đọc và chấp nhận các điều khoản trong <a
                                                href="user_terms_of_service.html">booking conditions</a>.</label>
                                    </div>
                                </div>
                    ...
                    @else
                        <p>Nếu bạn đã có tài khoản, hãy đăng nhập để đến bước tiếp theo <a class="login no-href">đăng
                                nhập</a> </p>
                        <form id="booking" method="post" action="{{ route('userBookingStep2') }}" class="booking">
                            {{ csrf_field() }}
                            <input type="hidden" name="homestay_id" value="{{ $homestay->id }}">
                            ...
                            <fieldset>
                                ...
                                <div class="row twins">
                                    ...
                                    <div class="f-item">
                                        ...
                                        @if ($errors->notLogin->has('phone'))
                                            <span
                                                class="error_account">{{ $errors->notLogin->first('phone') }}</span><br />
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="row twins">
                                    <div class="f-item">
                                        ...
                                        @if ($errors->notLogin->has('email'))
                                            <span
                                                class="error_account">{{ $errors->notLogin->first('email') }}</span><br />
                                        @endif
                                    </div>
                                    <div class="f-item">
                                        ...
                                        @if ($errors->notLogin->has('confirm_email'))
                                            <span
                                                class="error_account">{{ $errors->notLogin->first('confirm_email') }}</span><br />
                                        @endif
                                    </div>
                                ...
                                <div class="row">
                                    <div class="f-item checkbox" onclick="checked()">
                                        <input type="checkbox" name="check" id="check" value="ch1" />
                                        <label>Có, tôi đã đọc và chấp nhận các điều khoản trong <a
                                                href="{{ route('userConditions') }}">booking conditions</a>.</label>
                                    </div>
                                </div>
                                <input type="submit" class="gradient-button" disabled value="Tiếp theo" id="next-step" />
                            </fieldset>
                        ...
                        @if (Session::has('Cart-homestay-' . $homestay->id))
                            ...
                            @foreach (Session::get('Cart-homestay-' . $homestay->id)->product as $item)
                                <div class="booking-info">
                                    ...
                                    <h6 class="price-text">Giá: <span
                                            class="price">{{ number_format(($item['productInfo']->prices * (100 - $item['productInfo']->discount)) / 100, 0, ',', '.') }}đ</span>
                                    </h6>
                                </div>
                            @endforeach
                            <div class="price">
                                <p class="total">Tổng tiền: <span
                                        class="payment">{{ number_format(Session::get('Cart-homestay-' . $homestay->id)->totalPrice, 0, ',', '.') }}đ</span>
                                </p>
                                <p class="total">Tổng số phòng: <span
                                        class="payment">{{ Session::get('Cart-homestay-' . $homestay->id)->totalQuanty }}</span>
                                </p>
                                <p class="total">Ngày nhận phòng: <span class="payment"><i>{{ $datepicker1 }}</i> </span>
                                </p>
                                <p class="total">Ngày trả phòng: <span class="payment"> <i>{{ $datepicker2 }}</i> </span>
                                </p>
