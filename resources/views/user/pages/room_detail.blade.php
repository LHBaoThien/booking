            ...
            <div class="content clearfix">
                ...
                <nav role="navigation" class="breadcrumbs clearfix">
                    ...
                    <ul class="crumbs">
                        ...
                        <li><a href="{{ route('userSearch') . '?address=' . $homestayVal->province->name . $url }}"
                                title="Address">{{ $homestayVal->province->name }}</a></li>
                        <li><a href="{{ route('userSearch') . '?address=' . $homestayVal->district->name . ' - ' . $homestayVal->province->name . $url }}"
                                title="Address">{{ $homestayVal->district->name }}</a></li>
                ...
                <section class="three-fourth">
                        ...
                        <article>
                            ...
                            <div class="text-wrap">
                                @if (isset($datepicker1) && isset($datepicker2))
                                    <p>Phòng có sẵn từ <span class="date">{{ $datepicker1 }}</span> đến <span
                                            class="date">{{ $datepicker2 }}</span>.</p>
                                @endif
                            </div>
                            <!-- <h1>Room types</h1> -->
                            <ul class="room-types">
                                @foreach ($product as $productVal)
                                    <!--room-->
                                    <li>
                                        <figure class="left"><img src="{{ $productVal->avatar }}" alt="" width="270"
                                                height="152" />
                                            <a href="{{ $productVal->avatar }}" class="image-overlay"
                                                rel="prettyPhoto[gallery{{ $productVal->id }}]"></a>
                                        </figure>
                                        <div hidden>
                                            @foreach ($productVal->image as $imageProduct)
                                                <a href="{{ $imageProduct->url }}" class="image-overlay"
                                                    rel="prettyPhoto[gallery{{ $imageProduct->product_id }}]"></a></figure>
                                            @endforeach
                                        </div>
                                        <div class="meta">
                                        ...
                                        <div class="room-information">
                                            <div class="row">
                                                ...
                                                <span class="second"><span
                                                        class="capacity">{{ $productVal->roomType->capacity }} x</span><img
                                                        src="user/images/ico/person.png" alt="" /></span>
                                            </div>
                                            @if ($productVal->discount == 0)
                                                <div class="row price">
                                                    <span class="first">Giá: </span>
                                                    <span
                                                        class="second">{{ number_format($productVal->prices, 0, ',', '.') }}đ</span>
                                                </div>
                                            @else
                                                <div class="row price">
                                                    <span class="first">Giá: </span>
                                                    <span
                                                        class="second price-old"><strike>{{ number_format($productVal->prices, 0, ',', '.') }}</strike>đ</span>
                                                </div>
                                                <div class="row price">
                                                    <span class="first">&nbsp</span>
                                                    <span
                                                        class="second price-new">{{ number_format(($productVal->prices * (100 - $productVal->discount)) / 100, 0, ',', '.') }}đ</span>
                                                </div>
                                            @endif
                                            @if (isset($datepicker1) && isset($datepicker2))
                                                <a onclick="AddCart({{ $productVal->id }}, {{ $homestayVal->id }}, '{{ $datepicker1 }}', '{{ $datepicker2 }}')"
                                                    class="gradient-button no-href">Chọn</a>
                                            @else
                    ...
                    <section id="description" class="tab-content">
                        <article>
                            ...
                            <div class="text-wrap">
                                <p>Chính sách hủy khác nhau tùy theo loại phòng. Vui lòng kiểm tra các <a
                                        href="user_terms_of_service.html">điều kiện phòng </a>khi chọn phòng của bạn. </p>
                            </div>

                            <h1>Chính sách giành cho trẻ em</h1>
                            <div class="text-wrap">
                                <p><strong style="color:red;">Miễn phí!</strong> Với tất cả trẻ em dưới <strong
                                        style="color:#0005a0;">140</strong>cm.<br /><br />Tất cả trẻ em trên <strong
                                        style="color:#0005a0;">140</strong>cm đều được áp dụng chính sách dành cho người
                                    lớn<br /></p>
                            </div>
               ...
               <div class="pro">
                <div class="stars">
                 @for ($i = 5 - $ratingVal->point; $i--; $i >= 0)
                  <img src="user/images/ico/star-rating-off.png" alt="" /> @endfor
              ...
              <!--//review-->
                ...
                <aside class="right-sidebar">
                            ...
                            <ul>
                                @foreach ($homestayVal->image as $image)
                                    <li>
                                        ...
                                        <a href="{{ $image->url }}" class="image-overlay"
                        ...
                        <form action="{{ route('userRoomDetail') }}">
                            ...
                            <div class="row-3">
                                <div class="f-item spinner">
                                    ...
                                    <input type="text" placeholder="" min="1" value="{{ $num_room }}" id="num_room"
                    ...
                    <article class="default clearfix order">
                        ...
                        @if (isset($datepicker1) && isset($datepicker2))
                            <a href="{{ route('userBookingStep1') . '?id=' . $homestayVal->id . '&address=' . $homestayVal->province->name . $url }}"
                                class="gradient-button" title="Book">Thanh toán</a>
                        @endif
                    </article>
                    <!--// Booking?-->
                    ...
                    <article class="default clearfix">
                        ...
                        <p>Gọi cho nhóm dịch vụ khách hàng của chúng tôi theo số dưới đây để nói chuyện với một trong những
                            cố vấn của chúng tôi, những người sẽ giúp bạn với tất cả các nhu cầu kỳ nghỉ của bạn.</p>
