                ...
                <section class="three-fourth">
                    <form id="booking" method="post" action="booking" class="booking">
                        <fieldset>
                            ...
                            <h3>Thanh toán</h3>
                            <div class="text-wrap">
                                <h5>Khách hàng sẽ <strong class="dark">thanh toán trực tiếp</strong> tại
                                    Homestay<br /><br /></h5>
                                <p><strong class="dark">Mọi thông tin chi tiết đã được chúng tôi gửi đến hòm thư email của
                                        bạn</strong></p>
                            </div>
                            <br /><br />
                        ...
                        @foreach ($bill->order as $order)
                            <div class="booking-info">
                                ...
                                <h6 class="price-text">Giá: <span
                                        class="price">{{ number_format(($order->product->prices * (100 - $order->product->discount)) / 100, 0, ',', '.') }}đ</span>
                                </h6>
                            </div>
                        @endforeach
                        <div class="price">
                            <p class="total">Tổng tiền: <span
                                    class="payment">{{ number_format($bill->payments, 0, ',', '.') }}đ</span></p>
                            <p class="total">Tổng số phòng: <span class="payment">{{ $bill->order->count() }}</span></p>
                            <p class="total">Ngày nhận phòng: <span
                                    class="payment"><i>{{ date('d-m-Y', strtotime($bill->order->first()->date_start)) }}</i>
                                </span></p>
                            <p class="total">Ngày trả phòng: <span class="payment">
                                    <i>{{ date('d-m-Y', strtotime($bill->order->first()->date_end)) }}</i> </span></p>
