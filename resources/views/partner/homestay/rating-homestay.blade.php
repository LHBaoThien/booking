...
@section('main')
    ...
    <div class="main" role="main">
        <div class="wrap clearfix">
                        ...
                        <section id="MyBookings" class="tab-content tab-booking">
                            ...
                            @foreach ($rate as $rateVal)
                                ...
                                <article class="bookings ratings">
                                    <h1><strong class="gradient-button success stt ">{{ $i }}</strong> | Your
                                        review of <strong
                                            style="color: #dc3545;font-size: 18px;">{{ $rateVal->homestay->name }}</strong>
                                    </h1>
                                    <div class="b-info">
                                        ...
                                        <article class="myreviews ratings_homestay">
                                            ...
                                            <div class="reviews">
                                                ...
                                                @elseif($rateVal->point <3) <div class="con">
                                                        <p>{{ $rateVal->comment }}</p>
                                            </div>
                            @endif
                    </div>
            ...
            <div class="actions">
                @if ($rateVal->status == 1)
                    <a href="{{ route('ChangeStatus', ['bill_id' => $rateVal]) }}"
                        class="gradient-button danger">Change-status</a>
                @elseif($rateVal->status == 0)
                    <a href="{{ route('ChangeStatus', ['bill_id' => $rateVal]) }}"
                        class="gradient-button success">Change-status</a>
                @endif
            </div>
            </article>
            @endforeach
