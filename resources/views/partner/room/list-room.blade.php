            ...
            <div class="content clearfix">
                ...
                <section class="full">
                    ...
                    @if (empty(Auth::user()->phone))
                        <div class="alert"><i class="alert-danger">Vui lòng cập nhật số điện thoại trước khi tạo phòng</i>
                        </div>
                    ...
                    @else
                        <div class="sort-by" style="width:98%">
                            <ul class="sort custom" style="float: right">
                                <li>Thêm phòng
                                    <a href="{{ route('addRoom', ['id' => $homestay_id]) }}" title="addRoom"
                                        class="add"></a>
                                </li>
                            </ul>
                        </div>
                            ...
                            <div class="deals clearfix">
                                ...
                                @foreach ($product as $productVal)
                                    <article class="one-fourth">
                                        <figure><a href="{{ url('room-detail' . '?id=' . $productVal->homestay->id) }}"
                                                title=""><img src="{{ asset($productVal->avatar) }}" alt="" width="270"
                                                    height="152" /></a></figure>
                                        <div class="details">
                                            <h1>{{ $productVal->homestay->name }}</h1>
                                            <span class="address"> Tên • <a title="tên phòng"
                                                    style="color:red ;font-size :12px">{{ $productVal->name }}</a></span>
                                            <span class="address"> Loại • <a title="loại phòng"
                                                    style="color:red ;font-size :12px">{{ $productVal->roomType->name }}</a></span>
                                            ...
                                            <span class="price none-border">Thêm ảnh
                                                ({{ $productVal->image->count() }})&nbsp; &nbsp;
                                                <em>
                                                    <a href="{{ route('UploadImageRoom', ['id' => $productVal->id]) }}"><img
                                                            src="{{ URL::to('/') }}/partner/images/ico/plus.png" alt=""
                                                            width="22" height="22" /></a>
                                                </em>
                                            </span>
                                            <span class="price none-border" style="border-top : none;">Giá phòng
                                                <em>{{ number_format($productVal->prices, 0, ',', '.') }}đ</em> </span>
                                            <span class="price" style="border: none;margin : 0 auto"><label for="">Chỉnh sửa
                                                    &nbsp; &nbsp;</label>
                                                <em>
                                                    <a
                                                        href="{{ url('partner/edit-list-room', ['id' => $productVal->id]) }}"><img
                                                            src="{{ URL::to('/') }}/partner/images/ico/edit.png" alt=""
                                                            width="16" height="16" /></a>
                                                </em>
                                            </span>
                                            <a href="{{ route('delete_room', ['id' => $productVal->id]) }}" title="Xóa"
                                                class="gradient-button delete"
                                                onclick="return confirm('Bạn muốn xóa phòng ?')">Xóa phòng</a>
                                        </div>
                                    </article>
                                @endforeach
                    ...
                    <section class="full">
                        ...
                        <div class="deals clearfix">
                            ...
                            @foreach ($roomrestore as $ViewRoom)
                                <article class="one-fourth">
                                    <figure><a title=""><img src="{{ asset($ViewRoom->avatar) }}" alt="" width="270"
                                                height="152" /></a></figure>
                                    <div class="details">
                                        <h1>{{ $ViewRoom->homestay->name }}</h1>
                                        <span class="address"> Tên • <a title="tên phòng"
                                                style="color:red ;font-size :12px">{{ $ViewRoom->name }}</a></span>
                                        <span class="address"> Loại • <a title="loại phòng"
                                                style="color:red ;font-size :12px">{{ $ViewRoom->roomType->name }}</a></span>
                                        <span class="address status_room_custom">Trạng thái •
                                        ...
                                        @endif
                                        <span class="price none-border">Số lượng ảnh
                                            ({{ $ViewRoom->image->count() }})&nbsp; &nbsp;
                                        </span>
                                        <span class="price none-border" style="border-top : none;">Giá phòng
                                            <em>{{ number_format($ViewRoom->prices, 0, ',', '.') }}đ</em> </span>
                                        <a href="{{ route('Restore_Room', ['id' => $ViewRoom->id]) }}" title="Restore"
                                            class="gradient-button delete"
                                            onclick="return confirm('Bạn muốn khôi phục phòng chứ ?')">Khôi phục</a>
                                    </div>
                                </article>
                            @endforeach
                            <!--//deal-->
